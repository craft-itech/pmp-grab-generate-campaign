...
export class GenerateCampaignService {
  ...
  async onModuleInit() {
    ...
    setInterval(
      ...
      parseInt(process.env.BATCH_PERIOD),
    );
  }
  ...
  async groupPromotionsByMerchant(
    promotions: PromotionGrabmartEntity[],
  ): Promise<PromotionGrabmartEntity[][]> {
    const groupedPromotions: {
      [merchantID: string]: PromotionGrabmartEntity[];
    } = {};
  ...
  async checkCampaign() {
    ...
    const lastwait =
      updatestatus - 1000 * 60 * parseInt(process.env.BATCH_WAIT) * 1000;
    ...
    const sql =
      ...
      'FROM ' +
      process.env.TABLE_PROMOTION_GRABMART +
      ' ' +
    ...
    while (retry) {
      try {
        await this.promotionGrabmartRepository.query(sql, [
          ...
          process.env.BU,
          lastwait,
          parseInt(process.env.BATCH_SELLER_SIZE),
          parseInt(process.env.MAX_QUEUE),
          parseInt(process.env.MERCHANT_MOD),
          parseInt(process.env.MERCHANT_MOD_RES),
          formattedDate,
        ]);
        ...
        this.logger.warn(
          updatestatus + ' - Found error, wait 1000 ms and retry : ' + error,
        );
    ...
    this.logger.log(
      updatestatus + ' - Found promotion to process : ' + promotions.length,
    );
  ...
  async processNetCampaign(
    promotion: PromotionGrabmartEntity,
    updatestatus: number,
  ) {
    ...
    if (master) {
      if (master.sold_by_weight) {
        if (master.weight_unit === 'g') {
          promotion.campaign_value = Math.ceil(
            (parseInt(promotion.campaign_value) * master.weigth_value) / 1000,
          ).toString();
        } else {
          promotion.campaign_value = Math.ceil(
            parseInt(promotion.campaign_value) * master.weigth_value,
          ).toString();
        }
      }
      ...
      this.logger.debug(
        updatestatus +
          ...
          ' barcode ' +
          promotion.barcode +
          ' master not sync success ',
      );
    }
  }

  async processCampaignByMerchant(
    promotions: PromotionGrabmartEntity[],
    updatestatus: number,
  ) {
    this.logger.log(
      updatestatus +
        ' - Found remain promotion of merchant ' +
        promotions[0]?.merchant_id +
        ' to process : ' +
        promotions.length,
    );
    ...
    for (const promotion of promotions) {
      if (promotion.promotion_mode === 'DELETE') {
        ...
        this.logger.warn(
          updatestatus +
            ' - Failed to post campaign for merchant ID: ' +
            promotion.merchant_id +
            ' of ID ' +
            promotion.id +
            ' because end date already pass.',
        );
        ...
        for (const barcode of barcodes) {
          syncFinishCount =
            syncFinishCount +
            (await this.masterGrabmartRepository.count({
              where: {
                status: LessThan(0),
                barcode: barcode,
                seller_id: promotion.merchant_id,
        ...
        if (barcodes.length === syncFinishCount && barcodes.length > 0) {
          ...
          this.logger.debug(
            updatestatus +
              ...
              ' has ' +
              barcodes.length +
              ' barcode(s) but master sync success ' +
              syncFinishCount,
          );
        }
      }
  ...
  async readCampaign(merchantID: string) {
    ...
    const promotions: PromotionGrabmartEntity[] =
      await this.getPromotionsByMerchantId(merchantID);
    for (const promotion of promotions) {
  ...
  async createCampaign(
    ...
    this.logger.debug(
      updatestatus + ' - Create campaign : ' + JSON.stringify(grabCampaign),
    );
    try {
      ...
      if (response.status === 200) {
        if (response.data.status === 0) {
          ...
          this.logger.debug(
            updatestatus +
              ' - Successfully posted campaign for merchant ID: ' +
              merchantID +
              ' of ID ' +
              promotion.id +
              ' get campaign id: ' +
          ...
          this.logger.error(
            updatestatus +
              ' - Failed to posted campaign for merchant ID: ' +
              merchantID +
              ' of ID ' +
              promotion.id +
              ' error message ' +
        ...
        promotion.updated_date = new Date();
        promotion.error_msg =
          'Error status ' + response.status + ', please check log file.';
  ...
  async deleteCampaign(
    ...
    try {
      ...
      if (response.status !== 200) {
        ...
        this.logger.debug(
          updatestatus +
            ' - Successfully delete campaign for merchant ID: ' +
            merchantID +
            ' of ID ' +
            promotion.id +
            ' get campaign id: ' +
      ...
      if (isAxiosError(error)) {
        promotion.error_msg = JSON.stringify({
          error,
      ...
      this.logger.error(
        updatestatus +
          ' - Failed to delete campaign for campaign ID: ' +
          promotion.campaign_id +
          ' Error : ' +
          error,
      );
  ...
  async setGrabCampaign(
    entity: PromotionGrabmartEntity,
  ): Promise<GrabCampaignDto> {
  ...
  private async getConditions(
    ...
    conditions.startTime = this.utilService.checkAndAdjustDate(
      strStartDate,
      inputDateFormat,
      grabDateFormat,
      parseInt(process.env.ADJUST_TZ),
    );
    conditions.endTime = this.utilService.convertDateFormat(
      strEndDate,
      inputDateFormat,
      grabDateFormat,
      parseInt(process.env.ADJUST_TZ),
  ...
  async getPromotionsByMerchantId(
    merchantId: string,
  ): Promise<PromotionGrabmartEntity[]> {
